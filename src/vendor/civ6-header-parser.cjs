...
/**
 * Vendored from pydt/civ6-save-parser (MIT) — header-only marker scan.
 * https://github.com/pydt/civ6-save-parser
 */

const loggingEnabled = false;
const _ = {
  clone: (o) => (Array.isArray(o) ? [...o] : { ...o }),
...
function writeString(marker, newValue) {
  const safeValue = Buffer.from(String(newValue), 'ascii');
